...
public class SecurityConfiguration {
	...
	public SecurityFilterChain filterChain(HttpSecurity httpSecurity) throws Exception{
		DebugUsernamePasswordAuthFilter filter = new DebugUsernamePasswordAuthFilter();
	    filter.setAuthenticationManager(
	        httpSecurity.getSharedObject(AuthenticationManager.class)
	    );
	    filter.setFilterProcessesUrl("/users/loginProcess");
		return httpSecurity
				.cors(c -> c.disable())
				.csrf(c -> c.disable())
...
//																		"/wms_sem4/users/login",
																		"/wms_sem4/users/loginProcess")
												   .permitAll()
												   .requestMatchers("/wms_sem4/users/createCompanyStaff").hasAnyAuthority("ROLE_PORTAL_ADMIN", "ROLE_COMPANY_ADMIN")
												   .requestMatchers("/wms_sem4/users/**","/wms_sem4/companies/**").hasAnyAuthority("ROLE_PORTAL_ADMIN")
...
//										        response.sendRedirect("/users/login?error");
	...
	public ApplicationRunner runner(ApplicationContext ctx) {
	    return args -> {
	        ...
	        ctx.getBeansOfType(SecurityFilterChain.class)
